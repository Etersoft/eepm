...
for i in $BUILDROOT$PRODUCTDIR/libmip_*.so ; do
    [ -s "$i" ] || continue
    interpreter="$(a= patchelf --print-interpreter "$i" 2>/dev/null)" || continue
    [ "$interpreter" = "/lib/x86_64-linux-gnu/ld-linux-x86-64.so.2" ] && a= patchelf --set-interpreter /lib64/ld-linux-x86-64.so.2 "$i"
done
