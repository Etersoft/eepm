...
if [ "$(epm print info -p)" = "rpm" ] ; then
    ...
    PKGURL="$REPOURL/rpm/stable/x86_64/$(epm print constructname $PKGNAME "$FULLVERSION" x86_64 rpm)"
else
    # https://repo.yandex.ru/yandex-browser/deb/pool/main/y/yandex-browser-corporate/
    PKGURL="$REPOURL/deb/pool/main/y/$PKGNAME/$(epm print constructname $PKGNAME "$FULLVERSION" amd64 deb)"
fi
