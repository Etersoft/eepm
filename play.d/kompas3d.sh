...
case $(epm print info -e) in
    ...
    RedOS/8.0)
TMPREPO=$(mktemp -d)
cat > "$TMPREPO/ascon-beta.repo" << EOF
[ascon-beta]
name=Ascon Beta
baseurl=https://repo.ascon.ru/beta/rpm/redos/8.0
gpgkey=https://repo.ascon.ru/beta/rpm/ascon.gpg
gpgcheck=1
enabled=1
EOF
        epm repo add "file://$TMPREPO/ascon-beta.repo"
        ;;
    AstraLinuxSE/1.8)
        ...
        epm repo add "deb [signed-by=/etc/apt/trusted.gpg.d/ascon.gpg] https://repo.ascon.ru/beta/deb $(lsb_release -cs) main"
        ;;
    *)
